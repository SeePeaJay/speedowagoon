<template>
  <va-card title="Inoperable">
    <div class="row align--center mb-1">
      <div class="flex xs12 sm6">
        <va-input
          class="ma-0"
          :value="term"
          placeholder="Search by inoperable ID"
          @input="search"
          removable
        >
...
export default {
  ...
  computed: {
    ...
    filteredData () {
      ...
      return this.users.filter(item => {
        return item.inoperable_id.toLowerCase().startsWith(this.term.toLowerCase())
      })
    },
  },
  methods: {
  ...
  created () {
    db.collection('inoperable').get().then(
      querySnapshot => {
